qx.Class.define("elpintao.comp.deposito.pagePedidosSuc",
{
	...
	slbSucursal.addListener("changeSelection", function(e){
		...
		rpc.callAsync(function(resultado, error, id) {
			...
			if (resultado.length > 0) {
				...
				menutblPedido.memorizarEnabled([commandSeleccionar, btnImprimir, btnAnular], true);
			} else {
	...
	var btnAnular = new qx.ui.menu.Button("Anular...");
	btnAnular.setEnabled(false);
	btnAnular.addListener("execute", function(e){
		(new dialog.Confirm({
			"message"     : "Desea anular permanentemente el pedido de sucursal seleccionado?",
			"callback"    : function(e){
								if (e) {
									var rowData = tableModelPedido.getRowData(tblPedido.getFocusedRow());
									...
									var p = {};
									p.id_pedido_suc = rowData.id_pedido_suc;
	...
	*/
	...
	var tblSucursales = new componente.general.ramon.ui.table.Table(tableModelSucursales, custom);
